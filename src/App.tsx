...
type PresetProfile = {
  label: string;
  gainDb: number;
  presetEnabled: PresetSelectionState;
  presetStrength: PresetStrengthState;
  maxCombinedNoiseReduction: number;
  vocalBrightness: number;
  advancedCleanup: boolean;
};

const PRESET_PROFILES: Record<string, PresetProfile> = {
  Podcast: {
    label: "Podcast",
    gainDb: 6,
    presetEnabled: { voice_focused: true, chair_suppress: false, aggressive: false },
    presetStrength: { voice_focused: 60, chair_suppress: 0, aggressive: 0 },
    maxCombinedNoiseReduction: 70,
    vocalBrightness: 40,
  ...
  Aggressive: {
    label: "Aggressive",
    gainDb: 3,
    presetEnabled: { voice_focused: true, chair_suppress: true, aggressive: true },
    presetStrength: { voice_focused: 40, chair_suppress: 40, aggressive: 60 },
    maxCombinedNoiseReduction: 100,
    vocalBrightness: 30,
    advancedCleanup: true
  },
  Balanced: {
    label: "Balanced",
    gainDb: 0,
    presetEnabled: { voice_focused: true, chair_suppress: true, aggressive: false },
    presetStrength: { voice_focused: 40, chair_suppress: 35, aggressive: 25 },
    maxCombinedNoiseReduction: 80,
    vocalBrightness: 30,
    advancedCleanup: true
  },
  Custom: {
...
export default function App() {
  ...
  const [presetName, setPresetName] = useState<string>("Balanced");
  const [gainDb, setGainDb] = useState(PRESET_PROFILES["Balanced"].gainDb);
  const [presetEnabled, setPresetEnabled] = useState<PresetSelectionState>({ ...PRESET_PROFILES["Balanced"].presetEnabled });
  const [presetStrength, setPresetStrength] = useState<PresetStrengthState>({ ...PRESET_PROFILES["Balanced"].presetStrength });
  const [maxCombinedNoiseReduction, setMaxCombinedNoiseReduction] = useState(PRESET_PROFILES["Balanced"].maxCombinedNoiseReduction);
  const [vocalBrightness, setVocalBrightness] = useState(PRESET_PROFILES["Balanced"].vocalBrightness);
  const [advancedCleanup, setAdvancedCleanup] = useState(PRESET_PROFILES["Balanced"].advancedCleanup);
    ...
    function onPresetChange(e: React.ChangeEvent<HTMLSelectElement>) {
      ...
      if (profile) {
        ...
        setPresetEnabled({ ...profile.presetEnabled });
        setPresetStrength({ ...profile.presetStrength });
        setMaxCombinedNoiseReduction(profile.maxCombinedNoiseReduction);
        setVocalBrightness(profile.vocalBrightness);
        setAdvancedCleanup(profile.advancedCleanup);
      }
    }
  ...
  return (
    <main className="shell">
      <section className="card">
        ...
        {/* Preset Dropdown (applies to both modes) */}
        <div className="row">
          <label htmlFor="preset-profile">Preset Profile:&nbsp;</label>
          <select
            id="preset-profile"
            value={presetName}
          ...
          >
            {Object.keys(PRESET_PROFILES).map((key) => (
              <option key={key} value={key}>{PRESET_PROFILES[key].label}</option>
            ))}
          </select>
        </div>
